...
function renderTypeList(list) {
    ...
    cleanList.forEach((item) => {
        ...
        li.setAttribute("data-url", item.url);      // data-url stores additional information for developers to utilize
...
function typeClickedHandler(event) {
    ...
    // console.log(event.currentTarget);  // a more generic higher level target 
...
function renderPokeList(list) {
    ...
    // use a for each loop to print out the pokemon in each type below.
}
// set active item
function setActive(type) {
    const allTypes = document.querySelectorAll(".types > li"); // > in query represents direct descendent children
    allTypes.forEach((item)=>{
        if(item.dataset.url == type){
            item.classList.add("active"); // adds a class active to each class that meets the boolean and is associated with the items.
        }
        else {
...
function cleanTypeList(list) {
    return list.filter((item)=> item.name != "shadow" && item.name != "unknown"); // if true leave in if false filter out
}